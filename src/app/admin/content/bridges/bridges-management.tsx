...
type BridgesManagementProps = {
	initialContent: unknown;
};

export function BridgesManagement({ initialContent }: BridgesManagementProps) {
	...
	const handleSave = async () => {
		setSaving(true);
		try {
			const result = await saveSiteContent("page-bridges", content);
			if (result.success) {
				toast.success("Bridges page content saved");
			} else {
	...
	return (
		<div>
			<div className="mb-6 flex items-center justify-between">
				<div>
					<h2 className="text-2xl font-bold">Bridges Page Content</h2>
					<p className="text-muted-foreground text-sm">
						Edit the content displayed on the Bridges/Publications page
					</p>
				</div>
